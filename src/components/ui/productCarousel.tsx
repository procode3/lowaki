...
const ProductCarousel = ({ data}: {
	...
	return (
		<div>
			{/* Carousel container */}
			<div className="relative w-80 h-60 rounded-md overflow-hidden">
				{/* Image container */}
				<div
					ref={carouselRef}
					style={{
				...
				<div className=" absolute flex justify-between w-full top-[40%]">
					<button
						...
						className={`border px-4 py-2 mx-2 font-bold text-black bg-white/50 rounded-full ${
					...
					<button
						...
						onClick={() => setCurrentImg((prev) => prev + 1)}
						className={`border px-4 py-2 mx-2 font-bold text-black bg-white/50 rounded-full ${
							currentImg === data.images.length - 1 && "opacity-50"
						}`}
					>
						{">"}
					</button>
